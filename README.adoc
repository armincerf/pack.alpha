...
==== AOT compiled Uberjars

If you need to AOT compile your uberjar, you will need to run an AOT step to compile the application and generate the needed class files, then pass the directory containing those generated class files to pack using the `-e` flag. 
...
clojure -A:aot \
        -e \
'(binding [*compile-path* "build-data"]
  (compile (symbol "foo.main")))'

# Create an uberjar including the build data

clojure -Sdeps '{:deps {pack/pack.alpha {:git/url "https://github.com/juxt/pack.alpha.git" :sha "73552d02e43fa7fac4e3f245257742a9534ea0f2"}}}' -m mach.pack.alpha.one-jar -m foo.main -e "build-data" foo.jar

----
Note: the resulting class files will be placed in a `project.jar` within the lib directory of the generated jar
